...
-- Update chat_sessions policies to include auth users
...
CREATE POLICY "Users can update their own sessions" ON chat_sessions FOR UPDATE USING (auth.uid() = auth_user_id OR auth_user_id IS NULL);
...
CREATE POLICY "Users can update their own interactions" ON venue_interactions FOR UPDATE USING (auth.uid() = auth_user_id OR auth_user_id IS NULL);
