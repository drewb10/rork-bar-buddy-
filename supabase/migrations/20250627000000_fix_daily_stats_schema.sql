...
CREATE TABLE IF NOT EXISTS daily_stats (
  ...
  scoop_and_scores INTEGER DEFAULT 0 CHECK (scoop_and_scores >= 0),
...
DO $$ 
BEGIN
  -- Add scoop_and_scores column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'daily_stats' AND column_name = 'scoop_and_scores'
  ) THEN
    ALTER TABLE daily_stats ADD COLUMN scoop_and_scores INTEGER DEFAULT 0 CHECK (scoop_and_scores >= 0);
  END IF;
