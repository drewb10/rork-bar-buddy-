/*
  ...
  1. New Tables
    - `profiles` - User profiles with username and auth integration
    - Update existing tables to use auth.uid() references
...
CREATE OR REPLACE FUNCTION check_username_unique(new_username TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION check_username_unique(TEXT) TO anon, authenticated;