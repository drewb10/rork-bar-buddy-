/*
  ...
  1. Changes
     - Drops existing venue_interactions table to avoid conflicts
     - Creates a new venue_interactions table with proper schema
     - Adds support for both anonymous and authenticated users
  ...
  2. Schema
     ...
     - venue_id: TEXT for venue identifier
     - interaction_type: TEXT (default 'like')
     - arrival_time: TEXT for time slot
     - timestamp: TIMESTAMP WITH TIME ZONE
     - session_id: TEXT for optional session tracking
     - likes: INTEGER counter
     - created_at: TIMESTAMP WITH TIME ZONE
     - auth_user_id: UUID with foreign key to auth.users
*/
